...
from soccerpy import sock, sp_exceptions, handler
...
class Agent:
    def __init__(self):
        ...
        self.thinking = False # think thread and control variable
    ...
    def connect(self, host, port, teamname, version=15):
        """
        ...
        self.wm = WorldModel(handler.ActionHandler(self.sock))
        ...
        self.msg_thread = threading.Thread(target=self.message_loop,
                                           name="message_loop")
        ...
        # responses.
        init_address = self.sock.address
    ...
    def disconnect(self):
        ...
        if self.msg_thread.is_alive():
            self.msg_thread.join(0.01)

        if self.think_thread.is_alive():
            self.think_thread.join(0.01)
    ...
    def think(self):
        ...
        if not self.think_thread.is_alive() or not self.msg_thread.is_alive():
